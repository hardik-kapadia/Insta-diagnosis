...
def predict_covid_noncovid(img_path) -> str:
    ...
    path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "models/model_covid_noncovid.h5"
    )
...
def predict_brain_tumor(img_path):
    ...
    path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "models/brain_tumor_model.h5"
    )
    model = keras.models.load_model(path)
    ...
    return "{} with probability : {:.4f}%".format(
        labels[pred1[0]], pred[0, pred1[0]] * 100
    )
...
def predict_knee_arthritis(img_path):
    ...
    path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "models/knee_arthritis_model.h5"
    )
    ...
    pred1 = np.argmax(pred, axis=1)
    labels = ["0", "1", "2", "3", "4"]
    return "Grade {} with probability : {:.4f}%".format(
        labels[pred1[0]], pred[0, pred1[0]] * 100
    )
...
def predict_kidney_stone(img_path):
    ...
    path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "models/kidney_stone_model.h5"
    )
    model = keras.models.load_model(path)
